    ...
    <div className="min-h-screen bg-gray-50 py-8">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="mb-8">
          ...
          <p className="text-gray-600">Comprehensive management of all clients, bookings, and memberships</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
          ...
          <div className="bg-white rounded-lg shadow p-6">
            <div className="flex items-center">
              ...
              <div className="ml-4">
                <p className="text-sm font-medium text-gray-600">GCash Pending</p>
                <p className="text-2xl font-bold text-gray-900">{stats.pendingGcash}</p>
              </div>
            </div>
          </div>
          <div className="bg-white rounded-lg shadow p-6">
            <div className="flex items-center">
              ...
              <div className="ml-4">
                <p className="text-sm font-medium text-gray-600">Coach Payments</p>
                <p className="text-2xl font-bold text-gray-900">{stats.pendingCoachPayments || 0}</p>
              </div>
            </div>
          </div>
        </div>
                    ...
                    <tr key={client.email} className="hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4">
                        <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${
                          client.type === 'both' ? 'bg-purple-100 text-purple-800' :
                          client.type === 'member' ? 'bg-blue-100 text-blue-800' :
                          'bg-green-100 text-green-800'
                        }`}>
                          {client.type === 'both' ? '🎯 Both' : client.type === 'member' ? '🏋️ Member' : '📅 Booking Only'}
                        </span>
                      </td>
                      <td className="px-6 py-4 text-sm text-gray-900">{client.totalBookings}</td>
            ...
            <div className="bg-white rounded-lg shadow overflow-hidden">
              <div className="overflow-x-auto">
                <table className="min-w-full divide-y divide-gray-200">
                  <thead className="bg-gray-50">
                    <tr>
                      <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Customer & Activity</th>
                    ...
                    {filteredBookings.map((booking) => (
                      <tr key={booking.id} className="hover:bg-gray-50">
                        ...
                        <td className="px-6 py-4">
                          ...
                          <div className="text-xs text-gray-400">{booking.duration} hour{booking.duration > 1 ? 's' : ''}</div>
                        </td>
                        <td className="px-6 py-4">
                          <div className="text-sm text-gray-900">Ref: {booking.bookingReference}</div>
                          <div className="text-sm text-gray-500">Participants: {booking.participants}</div>
                        ...
                        <td className="px-6 py-4">
                          <div className="flex space-x-2">
                            {booking.status === 'pending' && (
                              <>
                                <button onClick={() => updateBookingStatus(booking.id, 'confirmed')} className="text-green-600 hover:text-green-900 bg-green-50 hover:bg-green-100 px-3 py-1 rounded text-xs font-medium">
                                  Confirm
                                </button>
                                <button onClick={() => updateBookingStatus(booking.id, 'cancelled')} className="text-red-600 hover:text-red-900 bg-red-50 hover:bg-red-100 px-3 py-1 rounded text-xs font-medium">
                                  Cancel
                                </button>
                              </>
                            )}
                            {booking.status === 'confirmed' && (
                              <>
                                <button onClick={() => updateBookingStatus(booking.id, 'completed')} className="text-blue-600 hover:text-blue-900 bg-blue-50 hover:bg-blue-100 px-3 py-1 rounded text-xs font-medium">
                                  Complete
                                </button>
                                <button onClick={() => updateBookingStatus(booking.id, 'cancelled')} className="text-red-600 hover:text-red-900 bg-red-50 hover:bg-red-100 px-3 py-1 rounded text-xs font-medium">
                                  Cancel
            ...
            <div className="bg-white rounded-lg shadow overflow-hidden">
              <div className="overflow-x-auto">
                <table className="min-w-full divide-y divide-gray-200">
                  <thead className="bg-gray-50">
                    <tr>
                      <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Member & Plan</th>
                      <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Membership Period</th>
                      ...
                      <tr key={membership.id} className="hover:bg-gray-50">
                        <td className="px-6 py-4">
                          <div className="flex items-center">
                            ...
                            <div className="ml-4">
                              ...
                              <div className="text-xs text-gray-400">{membership.phone}</div>
                        ...
                        <td className="px-6 py-4">
                          <div className="space-y-2">
                            <div className="text-sm font-medium text-gray-900">ID: {membership.membershipId}</div>
                        ...
                        <td className="px-6 py-4">
                          <div className="flex items-center mb-2">
                            ...
                            <span className="text-sm font-medium text-gray-700 capitalize">{membership.paymentMethod.replace('_', ' ')}</span>
                          </div>
                            ...
                            {(membership.status === 'payment_pending' || membership.status === 'pending') && (
                              <>
                                <button
                                  ...
                                  className="w-full bg-green-600 text-white px-3 py-2 rounded text-sm font-medium hover:bg-green-700 disabled:opacity-50"
                                >
                                ...
                                <button
                                  ...
                                  disabled={updatingId === membership.id}
                                  className="w-full bg-red-600 text-white px-3 py-2 rounded text-sm font-medium hover:bg-red-700 disabled:opacity-50"
                                >
                                  ❌ Reject
                                </button>
                              </>
        ...
        {activeTab === 'coach-bookings' && (
          <div>
            <div className="bg-white rounded-lg shadow overflow-hidden">
              <div className="px-6 py-4 border-b border-gray-200">
                <h2 className="text-lg font-semibold text-gray-900">Coach Session Bookings</h2>
                <p className="text-sm text-gray-500 mt-1">Manage and verify coach session payments</p>
              </div>
              <div className="overflow-x-auto">
                <table className="min-w-full divide-y divide-gray-200">
                  <thead className="bg-gray-50">
                    <tr>
                      <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Client & Coach</th>
                      ...
                      <tr key={booking.id} className="hover:bg-gray-50">
                        <td className="px-6 py-4">
                          <div className="space-y-2">
                            <div>
                              ...
                              <div className="font-medium text-gray-900">{booking.clientName || 'N/A'}</div>
                            ...
                            <div className="pt-2 border-t border-gray-100">
                              ...
                              <div className="font-medium text-blue-600">{booking.coachName}</div>
                              <div className="text-xs text-gray-500">{booking.coachEmail}</div>
                            </div>
                          </div>
                        </td>
                        <td className="px-6 py-4">
                            ...
                            <div className="max-h-32 overflow-y-auto space-y-1 mt-2">
                              {booking.sessions.map((session, idx) => (
                                <div key={idx} className="text-xs bg-gray-50 p-2 rounded">
                                  ...
                                  <div className="text-gray-600">{session.time} • {session.duration}h</div>
                                </div>
                              ))}
                            </div>
                          </div>
                        ...
                        <td className="px-6 py-4">
                          <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium border ${
                            ...
                            booking.status === 'paid' ? 'bg-purple-100 text-purple-800 border-purple-200' :
                            booking.status === 'pending_payment' ? 'bg-orange-100 text-orange-800 border-orange-200' :
                            booking.status === 'pending_confirmation' ? 'bg-yellow-100 text-yellow-800 border-yellow-200' :
                        ...
                        <td className="px-6 py-4">
                          <div className="flex flex-col space-y-2">
                            ...
                            {booking.status === 'confirmed' && (
                              <div className="text-center text-sm text-gray-600 bg-green-50 p-2 rounded border border-green-200">
                            ...
                            {booking.status === 'completed' && (
                              <div className="text-center text-sm text-gray-600 bg-blue-50 p-2 rounded border border-blue-200">
                                ✅ Completed
                              </div>
                            )}

                            {booking.status === 'cancelled' && (
                              <div className="text-center text-sm text-gray-600 bg-red-50 p-2 rounded border border-red-200">
                                ❌ Cancelled
                              </div>
                            )}
                ...
                {coachBookings.length === 0 && (
                  <div className="text-center py-12">
                    <p className="text-gray-500">No coach bookings yet</p>
                  </div>
                )}
              </div>
            </div>
